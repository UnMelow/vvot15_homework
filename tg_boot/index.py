...
# Константы и настройки окружения
...
def get_faces_without_name(session: ydb.Session):
    """
    Получает ключи лиц из базы данных, для которых не задано имя.

    :param session: Сессия подключения к базе данных YDB.
...
def update_face_name(session: ydb.Session, new_name: str, face_key: str) -> None:
    """
    Обновляет имя для заданного ключа лица в базе данных.

    :param session: Сессия подключения к базе данных YDB.
    :param new_name: Новое имя для лица.
    :param face_key: Ключ лица в базе данных.
    """
...
def send_message(chat_id: int, text: str, message_id: int) -> None:
    """
    Отправляет текстовое сообщение в Telegram.

    :param chat_id: ID чата в Telegram.
    :param text: Текст сообщения.
    :param message_id: ID сообщения, на которое нужно ответить.
    """
...
def send_photo(chat_id: int, img_key: str, message_id: int) -> None:
    """
    Отправляет фотографию в Telegram.

    :param chat_id: ID чата в Telegram.
    :param img_key: Ключ изображения для генерации URL.
    :param message_id: ID сообщения, на которое нужно ответить.
    """
...
def get_res(session: ydb.Session, sql_query: str):
    """
    Выполняет SQL-запрос к базе данных YDB и возвращает результаты.

    :param session: Сессия подключения к базе данных YDB.
    :param sql_query: Строка SQL-запроса.
    :return: Результат выполнения запроса.
    """
    try:
        return session.transaction().execute(
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Обрабатывает входящее событие от Telegram и выполняет соответствующие действия.

    :param event: Словарь с данными события.
    :param context: Контекст выполнения функции.
    :return: Словарь с информацией о статусе выполнения функции.
    """
