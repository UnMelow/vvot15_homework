...
provider "yandex" {
  service_account_key_file = "key.json"           # Service account key file
...
locals {
  service_account_id = jsondecode(file("key.json")).service_account_id # Extracting service account ID from key file
}
